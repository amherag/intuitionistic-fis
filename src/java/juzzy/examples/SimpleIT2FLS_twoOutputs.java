...
public class SimpleIT2FLS_twoOutputs 
{
    ...
    public SimpleIT2FLS_twoOutputs()
    {
        ...
        IntervalT2MF_Triangular smallSmileMF = new IntervalT2MF_Triangular("IT2MF for Small Smile", smallSmileUMF, smallSmileLMF);       
        ...
        IntervalT2MF_Triangular bigSmileMF = new IntervalT2MF_Triangular("IT2MF for Big Smile", bigSmileUMF, bigSmileLMF);
        ...
        IT2_Consequent smallSmile = new IT2_Consequent("SmallSmile", smallSmileMF, smile);
        IT2_Consequent bigSmile = new IT2_Consequent("BigSmile", bigSmileMF, smile);

        //Set up the rulebase and add rules
        rulebase = new IT2_Rulebase(4);
        rulebase.addRule(new IT2_Rule(new IT2_Antecedent[]{badFood, unfriendlyService}, new IT2_Consequent[]{lowTip, smallSmile}));
        rulebase.addRule(new IT2_Rule(new IT2_Antecedent[]{badFood, friendlyService}, new IT2_Consequent[]{mediumTip}));
        rulebase.addRule(new IT2_Rule(new IT2_Antecedent[]{greatFood, unfriendlyService}, new IT2_Consequent[]{lowTip}));
        rulebase.addRule(new IT2_Rule(new IT2_Antecedent[]{greatFood, friendlyService}, new IT2_Consequent[]{highTip, bigSmile}));
        ...
        plotControlSurface(tip, false, 100, 100);   //use COS type reduction
        plotControlSurface(smile, true, 100, 100);  //use centroid type reduction
    ...
    private void getTip(double foodQuality, double serviceLevel)
    {
        ...
        System.out.println("Using center of sets type reduction, the IT2 FLS recommends a "
                + "tip of: "+rulebase.evaluate(0).get(tip)+" and a smile of: "+rulebase.evaluate(0).get(smile));  
        System.out.println("Using centroid type reduction, the IT2 FLS recommends a "
                + "tip of: "+rulebase.evaluate(1).get(tip)+" and a smile of: "+rulebase.evaluate(1).get(smile));
    }
    ...
    private void plotControlSurface(Output o, boolean useCentroidDefuzzification, int input1Discs, int input2Discs)
    {
        ...
        plotter.plotControlSurface("Control Surface",
                new String[]{food.getName(), service.getName(), "Tip"}, x, y, z, o.getDomain(), true); 
